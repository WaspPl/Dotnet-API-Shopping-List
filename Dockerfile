...
WORKDIR /src
COPY ["Zaliczeniowy4.csproj", "."]
RUN dotnet restore "./Zaliczeniowy4.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./Zaliczeniowy4.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Zaliczeniowy4.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Zaliczeniowy4.dll"]